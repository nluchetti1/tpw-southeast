import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import cartopy.crs as ccrs
import cartopy.feature as cfeature
from satpy import Scene
import datetime
import xarray as xr
import numpy as np
import requests
import os
import imageio.v2 as imageio

# --- SETUP ---
now = datetime.datetime.utcnow().replace(minute=0, second=0, microsecond=0)
frames = []
num_steps = 10 

if not os.path.exists('frames'):
    os.makedirs('frames')

print(f"Generating Satellite Animation for last {num_steps} hours...")

for i in range(num_steps, -1, -1):
    target_time = now - datetime.timedelta(hours=i)
    time_str = target_time.strftime("%Y%m%d_%H00")
    
    tpw_data = None

    # 1. FETCH SATELLITE (MIMIC-TPW2)
    fname = f"{time_str}.nc"
    url = f"https://tropic.ssec.wisc.edu/archive/data/mtpw2/{target_time.year}/{fname}"
    local_nc = f"sat_{i}.nc"
    
    try:
        r = requests.get(url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=10)
        if r.status_code == 200:
            with open(local_nc, 'wb') as f:
                f.write(r.content)
            
            # Load and process via Satpy
            scn = Scene(reader='mimic_TPW2_nc', filenames=[local_nc])
            scn.load(['tpw'])
            ds_sat = scn.to_xarray_dataset()
            
            # Convert mm to inches
            tpw_data = ds_sat['tpw'] / 25.4 
            
            # Ensure correct orientation and regional slice
            if tpw_data.y[0] < tpw_data.y[-1]:
                tpw_data = tpw_data.sortby('y', ascending=False)
            tpw_data = tpw_data.sel(y=slice(42, 28), x=slice(-90, -70))
            print(f"  > Successfully processed Satellite: {time_str}Z")
    except Exception as e:
        print(f"  > Satellite download/process failed for {time_str}: {e}")

    # 2. PLOT FRAME
    if tpw_data is not None:
        fig = plt.figure(figsize=(12, 9), facecolor='black')
        ax = fig.add_subplot(1, 1, 1, projection=ccrs.PlateCarree())
        ax.set_extent([-88, -74, 31, 40]) # Southeast US focus

        # Satellite Plot (Inches)
        # Range 0.5 to 2.5 inches is typical for the SE US
        im = ax.pcolormesh(tpw_data.x, tpw_data.y, tpw_data, cmap='viridis', 
                           shading='auto', vmin=0.5, vmax=2.5)
        
        # Colorbar
        cb = plt.colorbar(im, ax=ax, orientation='vertical', pad=0.02, aspect=30)
        cb.set_label('Total Precipitable Water (inches)', color='white')
        cb.ax.yaxis.set_tick_params(color='white', labelcolor='white')

        # Geography Features (10m Resolution)
        ax.add_feature(cfeature.STATES.with_scale('10m'), edgecolor='white', linewidth=1.2)
        ax.add_feature(cfeature.COASTLINE.with_scale('10m'), edgecolor='cyan', linewidth=1.0)
        
        # High-res counties
        counties = cfeature.NaturalEarthFeature('cultural', 'admin_2_counties', '10m', facecolor='none')
        ax.add_feature(counties, edgecolor='gray', linewidth=0.4, alpha=0.5)

        plt.title(f"MIMIC-TPW Satellite (inches)\nValid: {target_time.strftime('%H:%MZ %d %b %Y')}", 
                  color='white', fontsize=14)
        
        frame_path = f"frames/frame_{i:02d}.png"
        plt.savefig(frame_path, facecolor='black', bbox_inches='tight', dpi=120)
        frames.append(imageio.imread(frame_path))
        plt.close()
    
    # Clean up local file to keep the runner tidy
    if os.path.exists(local_nc):
        os.remove(local_nc)

# 3. SAVE GIF & LATEST IMAGE
if frames:
    # Build animation
    imageio.mimsave('output_animation.gif', frames, fps=2, loop=0)
    # Save the most recent frame as a static map
    imageio.imwrite('output_map.png', frames[-1])
    print("Animation and Static Map generated successfully.")
else:
    print("No frames were generated. Check internet connectivity to tropic.ssec.wisc.edu.")
